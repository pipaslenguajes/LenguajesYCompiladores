...
asignacion:
	ID ASIG expresion_aritmetica { 
									if(es_long == 0)
										IndAsignacion = crearTerceto_cii("=", crearTerceto_ccc($1, "",""), IndExpresion); 
									else
									{
										IndAsignacion = crearTerceto_cii("=", crearTerceto_ccc($1, "",""), IndLong); 
										es_long = 0;
									}
								 };
...
expresion_logica: termino_logico { and_index++; saltos_and_a_completar[and_index] = -1; }
		 ...
		 | NOT { es_negado = 1; } termino_logico { es_negado = 0; };
		 ;
...
take:
	TAKE PA comp_aritmetico 							{
															Ind_take_op = crearTerceto_ccc("+", "", "");
														}
	PUNTO_COMA CTE_INT 									{
															Ind_take_comp = crearTerceto_icc('3', "", "");
															cant_rec=0;
															IndSuma = crearTerceto_icc(0,"","");
														}
	PUNTO_COMA CA lista_exp_punto_coma CC PC 			{
															printf("Regla 54: TAKE es TAKE PA comp_aritmetico PUNTO_COMA CTE_INT PUNTO_COMA CA lista_exp_punto_coma CC PC\n");
															int i;
															for (i=0; i<take_cant_saltos; i++) {
															char *salto = (char*) malloc(sizeof(int));
															itoa(terceto_index, salto, 10);
															tercetos[take_saltos_a_completar[i]].dos = salto;
														}
															IndTake = crearTerceto_icc(IndSuma,"","");
														}
...
lista_exp_punto_coma:
	lista_exp_punto_coma PUNTO_COMA expresion_aritmetica            {
																		...
																		IndListaTake = crearTerceto_cii("CMP", cant_rec,Ind_take_comp);
																		int IndPos;
																		IndPos= crearTerceto_ccc("BGT", "", "");
																		take_saltos_a_completar[take_cant_saltos] = IndPos;
																		take_cant_saltos++;
																		cant_rec++;
																		IndSuma=crearTerceto_cii("+",IndSuma,IndExpresion);
																	}
    | expresion_aritmetica                              			{
